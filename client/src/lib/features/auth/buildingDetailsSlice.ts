...
export const cancelReservation = createAsyncThunk(
  ...
  async ({ id, floorNumber, slotNumber, reservationIndex, vehicleType }: { id: string; floorNumber: number; slotNumber: number, reservationIndex: number, vehicleType: string }) => {
    const response = await axios.post(`http://localhost:5000/api/building/${id}/cancel`, { floorNumber, slotNumber, reservationIndex, vehicleType }, { headers: getHeaderToken() });
    return response.data;
  }
);
